...
**THIS IS OPTIONAL**. There are some other IDs that are useful when setting up Validator if you want to trigger leader-slot placement prior to receiving anyone else's stake. You need to create a stake accoun.

4) stake account ID - refered to ask `stake-account.json` lcoated `/home/sol` and will coordinate with `authority-keypair.json` to delegate to `vote-account-keypair.json`
...
```
dump this into start-validator.sh ( updated 02/07/2022):
```
#!/bin/bash
...
```
solana delegate-stake --stake-authority ~/authority-keypair.json DStjgNsHei9QavdcFTw3AreVznwmucNffdU8eN9XoDTv 9cEQH7KWA74Dktk3ZQhaWi6MirzcyH8mzA3iBTxyRHsE --fee-payer ~/validator-keypair.json
